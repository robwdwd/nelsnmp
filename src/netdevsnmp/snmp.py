...
INTEGRITY_ALGO = {"md5": cmdgen.usmHMACMD5AuthProtocol, "sha": cmdgen.usmHMACSHAAuthProtocol}
...
class SnmpHandler(object):
        ...
        for key in kwargs:
            ...
            if key == "port":
                try:
                    ...
                    self._raise_error(ArgumentError, "Port must be an integer between 1 and 65535")
        ...
        if self.version == "3":
            ...
            if self.level == "authNoPriv":
                self.snmp_auth = cmdgen.UsmUserData(
                    self.username, authKey=self.authkey, authProtocol=INTEGRITY_ALGO[self.integrity]
                )
    ...
    def get_value(self, *oidlist):
        ...
        cmdGen = cmdgen.CommandGenerator()
        errorIndication, errorStatus, errorIndex, varBinds = cmdGen.getCmd(
            self.snmp_auth, cmdgen.UdpTransportTarget((self.host, self.port)), *snmp_query, lookupMib=False
        )
    ...
    def set(self, oid=None, value=None, value_type=None, multi=None):
        ...
        errorIndication, errorStatus, errorIndex, varTable = cmdGen.setCmd(
            self.snmp_auth, cmdgen.UdpTransportTarget((self.host, self.port)), *snmp_sets, lookupMib=False
        )
