...
for input_file in input_files:
    with open(input_file, "r") as f:
        device_declarations = yaml.load(f.read(), Loader=Loader)
...
def declared_hostinfo(monkeypatch, request):
    data = [
        [ObjectName(".1.3.6.1.2.1.1.2.0"), ObjectIdentifier(request.param["sysobjectid"])],
        [ObjectName(".1.3.6.1.2.1.1.1.0"), OctetString(request.param["description"])],
    ]
    get_oids = request.param.get("get_oids")
    if get_oids:
        for entry in get_oids:
            data.append([ObjectName(get_oids[entry]["oid"]), OctetString(get_oids[entry]["value"])])
